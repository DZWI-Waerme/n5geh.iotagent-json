 ...
 */
function guessType(attribute, device, value = undefined) {
    if (device.active) {
...
function extractAttributes(device, current) {
    ...
    for (const k in current) {
        if (current.hasOwnProperty(k)) {
            values.push({
                name: k,
                type: guessType(k, device, current[k]),
                value: current[k]
            });
        }
    }
...
function singleMeasure(apiKey, deviceId, attribute, device, parsedMessage) {
    ...
    const values = [
        {
            name: attribute,
            type: guessType(attribute, device, parsedMessage[0]),
    ...
    for (let j = 0; j < messageObj.length; j++) {
        ...
        for (const i in measure) {
            if (measure.hasOwnProperty(i)) {
                values.push({
                    name: i,
                    type: guessType(i, device, measure[i]),
                    value: measure[i]
                });
            }
        }
