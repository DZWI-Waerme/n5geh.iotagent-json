 ...
 */
function guessType(attribute, device, measureType, value = undefined) {
    if (device.active) {
    ...
    if (payloadType && [constants.PAYLOAD_NGSIv2, constants.PAYLOAD_NGSILD].includes(payloadType.toLowerCase())) {
                ...
                if (entity.hasOwnProperty(k)) {
                    if (['id', 'type'].includes(k)) {
                        ...
                        if (payloadType.toLowerCase() === constants.PAYLOAD_NGSIv2) {
                            valuesEntity.push({
                                name: k,
                                type: guessType(k, device, entity[k].type, entity[k].value),
                            ...
                            if (k.toLowerCase() === '@context') {
                                ...
                                if (entity[k].type) {
                                    ent.type = guessType(k, device, entity[k].type, entity[k].value);
        ...
        for (const k in current) {
            if (current.hasOwnProperty(k)) {
                values.push({
                    name: k,
                    type: guessType(k, device, null, current[k]),
                    value: current[k]
                });
            }
        }
